...
public class DoublyLinkedList<E> implements List<E> {
	...
	public void swap(int pos1, int pos2) {
		if(pos1 >= 0 && pos2 >= 0 && pos1 < this.numElements && pos2 < this.numElements) {
	...
	public void replace(E oldElement, E newElement) {
		DNode<E> current = head;
		while(current != null) {
			if(current.getElement().equals(oldElement)) {
	...
	public void replaceRecurse(E oldElement, E newElement) {
		if(oldElement != null && newElement != null) {
			replacerecurse(this.head, oldElement, newElement);
		}
	}
	
	private void replacerecurse(DNode<E> current, E oldElement, E newElement) {
		if(current != null) {
			if(current.getElement().equals(oldElement)) {
				current.setElement(newElement);
			}
			
			replacerecurse(current.getNext(), oldElement, newElement);
		}
	}
